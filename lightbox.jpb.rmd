---
title: "analysis of images produced by SCS light box"
author: "Garett Heineck adapted by Jesse Puka-Beals"
...
*Garett's actions*
cropping images will reduce noise from light box and speed up processing time
added more training data


*to do*
improve training dataset
understand the advantages of binary vs categorical for detecting hulled vs dehulled seed
manually count images for number of hulled vs dehulled vs ergot and put that data into a spreadsheet so we can compare our algorithm with manually counted data


*instructions to analyze images prior to knitting*
on blue tablet...

Go to `C:\Users\pukab001\Pictures\OLYMPUS Capture\`

Copy the folder "IWG_img_output" and rename the copy with a unique name

Now double click the folder "IWG_img_output"

Delete all folders except c("original_img", "results", "training data")

Now double click on the folder "original_img"

Delete all photos within this folder

Drag/save any photos that you want to be analyzed by this script into this now empty "original_img" folder

Now select the results folder and open the excel document within, change to your specifications. See chunk 5 and chunk 12 to modify how the results folder can be combined with the data folder generated in this script.
...
```{r making folders so our images can have a home}
...
Now we load in a spreadsheet that has additional information about the images (i.e. plot number, treatment, spacing)
```{r image ID information, echo=F}
## Read in datasheet----
...
#************************#
# IWG_seed.dat<- read_excel(paste(img_IWG_seed.demo.1.30.21, "results", "IWG_results.xlsx", sep = "/"), 
#                               na = ".") %>%
#   mutate(sample = as.factor(rep),
#          plot_ID= as.factor(plot_ID))

read.csv(paste(img_IWG_seed.demo.1.30.21, "results", "practice.files.csv", 
               sep = "/"))[1:3,] %>%
...
```{r random forest prediction models}
...
#************************#
# Jesse messing around, trying to use same code for hulled and dehulled as ergot since it worked well

# palette_selection_dehulled.seed<- palette_selection_categorical %>%
#   mutate(classification = case_when(grepl("null", mix) ~ 0,
#                                     grepl("chaff", mix) ~ 0,
#                                     grepl("de", mix) ~ 1,
#                                     grepl("Hseed", mix) ~ 0,
...
#                          data=palette_selection_dehulled.seed, 
#                          ntree=30,
#                          mtry = 1,
#                          importance=TRUE)
...
# #************************#
# 
# palette_selection_hulled.seed<- palette_selection_categorical %>%
#   mutate(classification = case_when(grepl("null", mix) ~ 0,
#                                     grepl("chaff", mix) ~ 0,
#                                     grepl("de", mix) ~ 0,
#                                     grepl("Hseed", mix) ~ 1,
...
#                          data=palette_selection_hulled.seed, 
#                          ntree=30,
#                          mtry = 1,
#                          importance=TRUE)
...
palette_selection_dehulled.seed<- filter(palette_selection_binary, !grepl("hulled", mix)) %>%
  mutate(classification = case_when(grepl("null", mix) ~ 0,
                                    grepl("clean", mix) ~ 1))

palette_selection_dehulled.seed<- filter(palette_selection_binary, !grepl("clean", mix)) %>%
  mutate(classification = case_when(grepl("null", mix) ~ 0,
                                    grepl("hulled", mix) ~ 1))
...
importance(dehulled.seed.rfm)
#************************#
palette_selection_hulled.seed<- filter(palette_selection_binary, !grepl("clean", mix)) %>%
  mutate(classification = case_when(grepl("null", mix) ~ 0,
                                    grepl("hulled", mix) ~ 1))
...
```{r setting up file paths for data/image import and export}
...
for (i in 1:length(paths_cropped_IWG.seed)) {
  ...
  disc = makeBrush(19, "diamond") #setting the shape for adaptive thresholding*** #TWEAKED BY GARETT
  ...
  img.dehulled.seed.2<- dilate(img.dehulled.seed.1) #the dilate function will expand the masked area around the current prediction***
  img.dehulled.seed.3<- gblur(img.dehulled.seed.2, sigma = 2) > .5 #gblur will fill holes in object features then a global threshold (the proportion) will remove some low probability pixels from the mask***
  ...
  img.hulled.seed.2<- fillHull(img.hulled.seed.1) 
  img.hulled.seed.3<- erode(img.hulled.seed.2) #erode fucntion is the opposite of dilate so it constricts the mask***
  img.hulled.seed.4<- dilate(gblur(img.hulled.seed.3, sigma = 3) > .25)
  img.hulled.seed.overlay<- ImgOverlay(img.crop, img.hulled.seed.4)
  img.hulled.seed.paint<- paintObjects(img.hulled.seed.4, img.crop, col = '#4169e1', thick = T)
  ...
  img.ergot.2<- dilate(gblur(img.ergot.1, sigma = 3) > .4)
  img.ergot.overlay<- ImgOverlay(img.crop,img.ergot.2)
  img.ergot.paint<- paintObjects(img.ergot.2, img.crop, col = "#000080", thick = T, closed = T)
...
##check to see if there are now images in the folders
```

NOTE: when applying  Garett's training dataset on our images, it failed it ID any hulled seeds, resulting in the code breaking down because the 'mesur.hulled.seed.dat' dataframe was empty, and therefore couldn't be filtered by s.area. If all lines with hulled seed were commented out of the code then it ran fine. 

```{r loop 2 of 2}
## This sections will due fine tune predictions and generate the data which analyses can be conducted 

start<- Sys.time() #tracking time to completion***
for (i in 1:length(paths_cropped_IWG.seed)) {
  img.crop<-flop(rotate(readImage(paths_cropped_IWG.seed[i]), angle = 90))
  #************************#
  #************************#
  ...
  chaff.dat$thresh<- ifelse(chaff.dat$classify>0.10, #all pxiels above 0.1 probability of chaff, global thresholding is usually a pretty good idea***
                            chaff.dat$classify, 0)
  dehulled.seed.dat$classify<- predict(dehulled.seed.rfm, dehulled.seed.dat)
  dehulled.seed.dat$thresh<- ifelse(dehulled.seed.dat$classify>0.20, #all pxiels above 0.3 probability of dehulled seed*** #TWEAKED BY GARETT
                                    dehulled.seed.dat$classify, 0)
  hulled.seed.dat$classify<- predict(hulled.seed.rfm, hulled.seed.dat)
  hulled.seed.dat$thresh<- ifelse(hulled.seed.dat$classify>0.20, #all pxiels above 0.1 probability of hulled seed*** #TWEAKED BY GARETT
                                  hulled.seed.dat$classify, 0)
  ergot.dat$classify<- predict(ergot.rfm, ergot.dat)
  ergot.dat$thresh<- ifelse(ergot.dat$classify>0.8, #all pxiels above 0.1 probability of ergot***  #TWEAKED BY GARETT
  ...
  wtrshed.chaff<- flop(rotate(watershed(img.chaff, 
                                 ext = 15),angle=90))  #TWEAKED BY GARETT
  ...
  wtrshed.dehulled.seed<- flop(rotate(watershed(distmap(img.dehulled.seed), 
                                 ext = 1, #TWEAKED BY GARETT
                                 tolerance = 1), angle=90)) #TWEAKED BY GARETT, added in a rotate arg after flop and angle =90 arg 
  ...
  mesur.dehulled.seed.dat<- data.frame(computeFeatures.shape(wtrshed.dehulled.seed, ref=img.crop)) %>%
    filter(s.area > 100) # tweaked by garett
  #************************#
  ...
  wtrshed.hulled.seed<- flop(rotate(watershed(distmap(img.hulled.seed),
                                 ext = 3, tolerance = 3), angle=90)) #TWEAKED BY GARETT, changed extension from 7 to 3, added tolerance arg and rotate arg
...
```{r stats}
## Data analysis, sing the output and do a little dance
...
img.stats.tidy <- dat %>%
  select(ends_with(c("objects","object")),
         filename) %>%
  rename(chaff=chaff.total.objects,
         dehulled=dehulled.total.objects,
         hulled=hulled.total.objects,
         ergot=ergot.total.object) %>%
  mutate(method="computer")


iwg.stats.manual <-IWG_seed.dat %>%
  mutate(method="human") %>%
  select(-tally,-X)

dat1<-rbind(img.stats.tidy, iwg.stats.manual);head(dat1)
# dat1$trainingdata <- paste(mixes_names,collapse = ",") 
# dat1$trainingdata[1] # logs training data used to generate estimates
dat1$trainingdatafiles<-length(mixes_names);dat1

dat1
# write.csv(dat1,paste("counting.performance",dat1$trainingdatafiles[1],"BinYes","csv", 
#       sep = ".")) #added "BinYes to show I was using the Bin_ files rather than Cat_

# anova(lm(dehulled~method,data=dat1))
`r format(Sys.Date(), "%m %d %Y")`

dat1 %>% 
  mutate(run_date=Sys.Date(),
         run_time=Sys.time()) %>% 
  # write.csv("test-me.csv")
  write.csv(paste("image-outputs",
                  Sys.Date(),
                  paste0(format(Sys.time(),"%H-%M-%S"),".csv"),
                  sep = "_"),
            row.names = F)

```

```{r combining previous datasets for training data comparison, eval=F}
read.csv(paste("counting.performance",15,"csv",sep = "."))
read.csv(paste("counting.performance",27,"csv",sep = "."))
dat2<-rbind(read.csv(paste("counting.performance",15,"csv",sep = ".")),
      read.csv(paste("counting.performance",27,"csv",sep = "."))
      );dat2
for(i in 1:length(dat2$trainingdatafiles)){
  dat2[i,8]<-dat2[i,8]-6
} # for loop removes the 6 Bin_ files that were counted as trainingfiles in the first 2 counting.performance.csv files I made. I am doing this just to show that we are using different amounts of training files. 
dat2

dat3<-rbind(dat2,
      read.csv(paste("counting.performance",27,"BinYes","csv",sep = ".")),
      read.csv(paste("counting.performance",31,"BinYes","csv",sep = ".")));dat3


dat3 %>%
  filter(method=="computer") 

dat3 %>%
  arrange(method) %>%
  slice_head(n=15)

for (i in 1:length(dat3$method)){
  ifelse(dat3[i,7]=="human",
         dat3[i,8]<-NA,
         dat3[i,8]<-dat3[i,8])
}
...
dat3$Bin_ <- NA
for (i in 1:length(dat3$method)){
  ifelse(dat3[i,8]==27|dat3[i,8]==31,
         dat3[i,9]<-T,
         dat3[i,9]<-F)
}
...
dat3 %>%
  arrange(method) %>%
  slice_head(n=15) %>%
  relocate(filename,.after="Bin_") %>%
  relocate(c(method,trainingdatafiles))
  select(-X)

anova(lm(dehulled~trainingdatafiles,data = subset(dat2,method=="computer")))
anova(lm(hulled~trainingdatafiles,data = subset(dat2,method=="computer")))


# anova(lm(dehulled~method,data=dat2))
...
# population.hulled.lm<- lm(hulled.total.objects~row_spacing, data = dat)
# anova(population.hulled.lm)
# population.hulled.emm<- data.frame(emmeans(population.hulled.lm, ~ row_spacing))
# 
# ggplot(population.hulled.emm, aes(x=as.factor(row_spacing), y=emmean)) +
#   geom_col(color="black",
#            fill="dodgerblue3", size=1)+
#   geom_errorbar(aes(ymin=emmean-SE, ymax=emmean+SE))+
#   labs(x="population", y="hulled seeds per 15 ml seed (+/-SE)")+
...
```{r picturs for markdown}
# plot(img.dehulled.seed.paint)
